...
if(isset($_POST['SavingTarget'])){
    $saveTarget = $_POST['SavingTarget'];
    mysqli_query($conn, "UPDATE accounts SET SavingTarget='$saveTarget' WHERE AccountNo = '$AcNo'") or die(mysqli_error($conn));
    echo('success');
...
if (isset($_POST['Amount'])){
    ...
    if (mysqli_num_rows($result)>0){
        while ($row = mysqli_fetch_assoc($result)){
            $balance = $row['Balance'];
            $sbalance = $row['SavingBalance'];
        }
        if ($balance>0 && $balance>$amount){
            $newBalance = $balance - $amount;
            $newSBalance = $sbalance + $amount;

            mysqli_query($conn, "UPDATE accounts SET Balance='$newBalance' WHERE AccountNo = '$AcNo'") or die(mysqli_error($conn));
            mysqli_query($conn, "UPDATE accounts SET SavingBalance='$newSBalance' WHERE AccountNo = '$AcNo'") or die(mysqli_error($conn));
            // mysqli_query($conn, "INSERT INTO transaction(AccountNo, FAccountNo, Name, Amount, Status, ProfileColor, Credit, Debit) VALUES ('$AcNo', '','SELF', '$amount', 'Savings', '$SProColor',$amount,'0.0')");
